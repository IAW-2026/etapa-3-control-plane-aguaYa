import { sellerApi } from "@/lib/api"
import { Store, Search } from "lucide-react"

type Vendor = {
  id: string
  name: string
  description?: string
  address: string
  isActive: boolean
  userId: string
  products?: { id: string }[]
  orders?: { id: string }[]
}

export const dynamic = "force-dynamic"

export default async function VendorsPage() {
  let vendors: Vendor[] = []
  let error: string | null = null

  try {
    vendors = await sellerApi.get("/api/admin/vendors")
  } catch (e) {
    error = e instanceof Error ? e.message : "Error al cargar vendedores"
  }

  return (
    <div>
      <div className="mb-8 flex items-center justify-between">
        <h1 className="text-2xl font-bold text-slate-900">Vendedores</h1>
      </div>

      {error && (
        <div className="mb-6 rounded-lg border border-red-200 bg-red-50 p-4 text-sm text-red-700">
          {error}
        </div>
      )}

      <div className="rounded-xl border border-slate-200 bg-white shadow-sm">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-200 text-left">
                <th className="px-6 py-4 font-medium text-slate-500">Nombre</th>
                <th className="px-6 py-4 font-medium text-slate-500">Dirección</th>
                <th className="px-6 py-4 font-medium text-slate-500">Productos</th>
                <th className="px-6 py-4 font-medium text-slate-500">Pedidos</th>
                <th className="px-6 py-4 font-medium text-slate-500">Estado</th>
              </tr>
            </thead>
            <tbody>
              {vendors.length === 0 && !error && (
                <tr>
                  <td colSpan={5} className="px-6 py-12 text-center text-slate-400">
                    No hay vendedores registrados
                  </td>
                </tr>
              )}
              {vendors.map((vendor) => (
                <tr key={vendor.id} className="border-b border-slate-100 hover:bg-slate-50">
                  <td className="px-6 py-4 font-medium text-slate-900">{vendor.name}</td>
                  <td className="px-6 py-4 text-slate-500">{vendor.address}</td>
                  <td className="px-6 py-4 text-slate-500">{vendor.products?.length ?? 0}</td>
                  <td className="px-6 py-4 text-slate-500">{vendor.orders?.length ?? 0}</td>
                  <td className="px-6 py-4">
                    <span
                      className={`inline-flex rounded-full px-2.5 py-0.5 text-xs font-medium ${
                        vendor.isActive
                          ? "bg-emerald-100 text-emerald-700"
                          : "bg-red-100 text-red-700"
                      }`}
                    >
                      {vendor.isActive ? "Activo" : "Inactivo"}
                    </span>
                  </td>
                </tr>
              ))}
            </tbody>
          </table>
        </div>
      </div>
    </div>
  )
}
