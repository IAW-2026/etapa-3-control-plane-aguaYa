...
import { getPaymentAdmins, deletePaymentAdmin } from "@/lib/actions/payment-admin"
import type { PaymentAdmin, ListResponse } from "@/lib/types"
import { Search, ChevronLeft, ChevronRight, Loader2, Shield } from "lucide-react"
import { Trash2 } from "lucide-react"
import Link from "next/link"
import CreatePaymentAdminWrapper from "@/components/payments/CreatePaymentAdminWrapper"

const PAGE_SIZE = 10

export default function PaymentAdminsPage() {
  const [data, setData] = useState<ListResponse<PaymentAdmin> | null>(null)
  ...
  const load = useCallback(async (p: number, s: string, tab: string) => {
    ...
    try {
      ...
      const res = await getPaymentAdmins(params)
      setData(res)
    } catch (e) {
  ...
  return (
    <div>
      <div className="mb-8 flex items-center justify-between">
        <div>
          <p className="text-sm font-semibold uppercase tracking-[0.24em] text-sky-700 dark:text-sky-400">Payment App</p>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-100">Admin Pagos</h1>
        </div>
        <CreatePaymentAdminWrapper onCreated={() => setReloadKey((k) => k + 1)} />
      </div>
        ...
        <div className="overflow-x-auto">
          {loading ? (
            ...
            <div className="flex flex-col items-center gap-2 py-16 text-sm text-slate-400">
              ...
              <p>No hay administradores de pagos registrados</p>
            </div>
          ) : (
                ...
                {data.items.map((admin) => (
                  <tr key={admin.clerkUserId} className="transition-colors hover:bg-white/20 dark:hover:bg-white/5">
                    <td className="px-6 py-4">
                      <Link href={`/dashboard/payment-admins/${admin.clerkUserId}`} className="font-medium text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300">
                        {admin.email}
                      </Link>
                    </td>
