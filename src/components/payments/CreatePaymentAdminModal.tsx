...
import { createPaymentAdmin } from "@/lib/actions/payment-admin"
...
export default function CreatePaymentAdminModal({ isOpen, onClose, onSuccess }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true)
    try {
      await createPaymentAdmin({ email, password, nombre, telefono: telefono || undefined })
      showToast("success", "Admin de pagos creado correctamente")
      onClose()
      onSuccess()
    } catch (e) {
      ...
      if (msg.toLowerCase().includes("ya existe") || msg.toLowerCase().includes("already exists")) {
        ...
        showToast("error", `Error al crear admin de pagos: ${msg}`)
      }
    } finally {
      setSaving(false)
    }
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Nuevo Admin de Pagos">
      <form onSubmit={handleSubmit} className="space-y-4 text-sm">
        ...
        <div className="flex justify-end gap-2 border-t border-white/20 pt-4 dark:border-slate-700/30">
          ...
          >
            ...
            {saving ? "Creando..." : "Crear Admin de Pagos"}
          </button>
        </div>
      </form>
    </Modal>
