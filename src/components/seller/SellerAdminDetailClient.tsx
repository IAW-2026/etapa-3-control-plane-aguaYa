...
export default function SellerAdminDetailClient({ admin: initial }: { admin: SellerAdmin }) {
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    setSaving(true)
    try {
      const updated = await updateSellerAdmin(admin.clerkUserId, {
        nombre: editNombre || undefined,
        telefono: editTelefono || undefined,
      })
      setAdmin(updated)
      setEditOpen(false)
      showToast("success", "Admin seller actualizado")
    } catch {
      showToast("error", "Error al actualizar admin seller")
    } finally {
      setSaving(false)
    }
  }
  ...
  async function handleDelete() {
    setDeleting(true)
    try {
      await deleteSellerAdmin(admin.clerkUserId)
      showToast("success", "Admin seller eliminado")
      router.push("/dashboard/seller-admins")
    } catch {
      showToast("error", "Error al eliminar admin seller")
      setDeleting(false)
      setDeleteOpen(false)
    }
  }
  ...
  return (
    <div>
      <div className="mb-6">
        <Link
          href="/dashboard/seller-admins"
          className="inline-flex items-center gap-1 text-sm text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200"
        >
          <ChevronLeft className="h-4 w-4" />
          Volver a admin sellers
        </Link>
      </div>
      ...
      <div className="rounded-xl border border-white/30 bg-gradient-to-br from-white/30 to-slate-100/30 shadow-lg shadow-black/5 backdrop-blur-xl dark:border-slate-700/40 dark:from-slate-900/40 dark:to-slate-800/40">
        <div className="flex items-start justify-between border-b border-white/20 px-6 py-5 dark:border-slate-700/30">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">Admin Seller — Gestión global</p>
          </div>
      ...
      {editOpen && (
        <Modal isOpen={editOpen} onClose={() => setEditOpen(false)} title="Editar Admin Seller">
