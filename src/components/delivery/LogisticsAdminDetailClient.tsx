...
import { toggleLogisticsAdmin, updateLogisticsAdmin, deleteLogisticsAdmin } from "@/lib/actions/delivery"
...
export default function LogisticsAdminDetailClient({ admin: initial }: { admin: LogisticsAdmin }) {
  ...
  function openEdit() {
    ...
    setEditNombreEmpresa(admin.nombreEmpresa)
    setEditOpen(true)
  }
  ...
  async function handleToggle() {
    setLoading(true)
    try {
      const res = await toggleLogisticsAdmin(admin.clerkUserId)
      setAdmin((prev) => ({ ...prev, isBlocked: res.nuevoEstado === "blocked" }))
      showToast("success", `Administrador ${res.nuevoEstado === "blocked" ? "bloqueado" : "desbloqueado"}`)
    } catch {
      showToast("error", "Error al cambiar estado del administrador")
    } finally {
      setLoading(false)
    }
  }
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    setSaving(true)
    try {
      const updated = await updateLogisticsAdmin(admin.clerkUserId, {
        nombre: editNombre,
        nombreEmpresa: editNombreEmpresa,
      })
      setAdmin(updated)
      setEditOpen(false)
      showToast("success", "Administrador actualizado")
    } catch {
      showToast("error", "Error al actualizar administrador")
    } finally {
      setSaving(false)
    }
  }
  ...
  async function handleDelete() {
    setDeleting(true)
    try {
      await deleteLogisticsAdmin(admin.clerkUserId)
      showToast("success", "Administrador eliminado")
      router.push("/dashboard/logistics-admins")
    } catch {
      showToast("error", "Error al eliminar administrador")
      setDeleting(false)
      setDeleteOpen(false)
    }
  }
  ...
  return (
    <div>
      <div className="mb-6">
        <Link
          href="/dashboard/logistics-admins"
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-800">
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">{admin.nombreEmpresa}</p>
          </div>
        ...
        <div className="mt-8 grid grid-cols-1 gap-6 sm:grid-cols-2">
          ...
          <div>
            <p className="text-xs font-medium text-slate-400 uppercase">Empresa</p>
            <p className="mt-1 text-sm text-slate-900 dark:text-slate-100">{admin.nombreEmpresa}</p>
          </div>
          <div>
            <p className="text-xs font-medium text-slate-400 uppercase">ID Vendedor</p>
            <p className="mt-1 font-mono text-sm text-slate-900 dark:text-slate-100">{admin.idVendedor}</p>
          </div>
          <div>
      ...
      {editOpen && (
        <Modal isOpen={editOpen} onClose={() => setEditOpen(false)} title="Editar Administrador">
          <form onSubmit={handleSave} className="space-y-4 text-sm">
            ...
            <div>
              <label className="mb-1 block text-xs font-medium text-slate-500 dark:text-slate-400">Nombre de empresa</label>
              <input
                type="text"
      ...
      <ConfirmDialog
        ...
        message={`¿Estás seguro de eliminar a "${admin.nombre}"? Se le quitarán los permisos de administrador. Esta acción no se puede deshacer.`}
